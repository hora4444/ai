# 가상환경생성 방법1 : python -m venv . venv

# 가상환경생성 방법2 : ctrl + shift + p => select interpreter => 가상환경만들기
                    # => .venv로 가상환경만들기 => 인터프리터경로입력 => 찾기(python.exe) [경로 = c:/사용자/pc명/anaconda3 에 있음]
...
app = Flask(__name__)
# 필터링 추가(str->str 문자갯수만큼 *로 전환)
app.template_filter('mask_pw')(mask_password)

@app.route('/user/<name>')
def viewFunction_handlerFunction(name): # /user/hong (동적 라우팅 : update, delete 할 때)
    return f'<h1>{name}님 환영합니다.</h1>'
@app.route('/user')                     # /user?name=hong (정적 라우팅 : read일 때)
def get_user():
    name =request.args.get('name') # get방식 파라미터 값 받기
    if name:
        return f'<h1>전달발은 이름 {name}님 반갑습니다.</h1>'
    else:
        abort(404)
# 에러페이지 처리(예외페이지 처리)
