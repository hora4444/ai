...
def split_rect_vertical(rect: fitz.Rect, parts: int = 3, overlap_ratio: float = 0.02):
    """
    rect를 세로(y축)로 parts등분하고, 경계에 overlap을 조금 준다.
    overlap_ratio: rect 높이 대비 비율 (0.02 = 2%)
    """
    h = rect.height
    if parts <= 1 or h <= 0:
        return [rect]
...
def render_solution_images(pdf_path: str, out_dir: Path, *, dpi: int = 200):
    ...
    try:
        ...
        # 해설은 우선 "flow" segment 사용 (너 파일에 이미 있음)
        segs = build_solution_segments_flow(
            doc, anchors,
            pad=12, content_top=140, content_bottom_margin=140
        )
        ...
        for qnum, rects in segs.items():
            ...
            for (pno, rect) in rects:
                ...
                # ✅ 여기서 3분할
                subrects = split_rect_vertical(rect, parts=3, overlap_ratio=0.02)

                for idx, sub in enumerate(subrects, start=1):
                    sub = sub & page.rect
                    if not _is_valid_rect(sub):
                        continue

                    pix = page.get_pixmap(clip=sub, dpi=dpi)
...
def render_question_images(pdf_path: str, out_dir: Path, *, dpi: int = 200):
    ...
    try:
        ...
        segs = build_question_segments(
            doc, anchors,
            column_mode="fixed",
            pad=6, content_top=90, content_bottom_margin=90,
        )
...
def render_exam_images(pdf_path: str, out_dir: Path, *, dpi: int = 200, kind: str = "question"):
    if kind == "solution":
        return render_solution_images(pdf_path, out_dir, dpi=dpi)
    return render_question_images(pdf_path, out_dir, dpi=dpi)
