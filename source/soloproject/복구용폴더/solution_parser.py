...
def ask_ollama_vision(img_input):
    ...
    prompt = """
    당신은 수학 해설 전문 조판사입니다. 
    이미지에 보이는 수학 문제의 해설과 수식을 완벽한 LaTeX 코드로 변환하십시오.
    
    [엄격 준수 규칙]
    1. Python 코드를 절대 작성하지 마십시오. (예: import, numpy 사용 금지)
    2. 설명이나 인사말 없이 오직 LaTeX와 Markdown 텍스트만 출력하십시오.
    3. 모든 수식은 $...$ 또는 $$...$$ 형식을 사용하십시오.
    4. 여러 줄의 식 전개는 \\begin{aligned} ... \\end{aligned}를 사용하고 등호(&=)를 정렬하십시오.
    5. 한글 텍스트는 원문의 의미를 살려 그대로 유지하십시오.
    """
...
def process_pdf_page_to_granite(pdf_path, page_num):
    if not PDF_AVAILABLE:
        print("      [!] PyMuPDF(fitz)가 설치되어 있지 않습니다.")
        return ""
    ...
    # 사용자 지정 좌표 적용 (상 900, 하 850, 좌우 300 제외)
    # ※ 주의: PDF 포인트 단위에 맞춰 조정이 필요할 수 있습니다.
    crop_rect = fitz.Rect(300, 900, rect.width - 300, rect.height - 850)
    
    col_width = crop_rect.width / 3
    page_results = []

    print(f"    [>] 페이지 {page_num+1}의 3열 분할 처리를 시작합니다...")
    
    for i in range(3):
        # 각 열의 영역 계산
        col_rect = fitz.Rect(crop_rect.x0 + (col_width * i), crop_rect.y0, 
                             crop_rect.x0 + (col_width * (i+1)), crop_rect.y1)
        
        # 고해상도(Matrix 2.0)로 이미지 추출
        pix = page.get_pixmap(clip=col_rect, matrix=fitz.Matrix(2, 2))
        img_pil = Image.open(io.BytesIO(pix.tobytes("png")))
        
        print(f"      - {i+1}번째 열 변환 중...")
        # 기존의 스마트 분할 로직(process_solution_with_splitting)에 이미지 전달
        # 여기서는 편의상 바로 모델을 호출하거나, 필요시 더 쪼개는 로직을 연결합니다.
        col_latex = ask_ollama_vision(img_pil)
        if col_latex:
            page_results.append(col_latex)
