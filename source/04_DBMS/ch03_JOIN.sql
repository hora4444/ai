...
-- ★ <총 연습문제> Part1
    ...
    SELECT ENAME, DNAME, LOC 
        FROM EMP E, DEPT D 
        WHERE E.DEPTNO = D.DEPTNO AND COMM > 0; -- NULL도 걸러진다.
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, MANAGER.ENAME "MANAGER"
    ...
    SELECT W.ENAME ||'의 상사는 ' || M.ENAME || '다'
        FROM EMP W, EMP M
        WHERE W.MGR=M.EMPNO;
    --탄탄EX. 매니저가 KING인 사원들의 이름과 직급을 푹력
    SELECT W.ENAME, W.JOB
        FROM EMP W, EMP M
        WHERE W.MGR = M.EMPNO AND M.ENAME='KING';

-- 4. OUTER JOIN : EQUI/ SELF JOIN시 조건에 만족하지 않는 행까지 나타나게
...
-- ★ <총 연습문제> Part2
    ...
    SELECT W.ENAME, GRADE, DNAME, M.ENAME "MANAGER"
