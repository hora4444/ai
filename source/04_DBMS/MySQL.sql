...
-- 기존 데이터베이스들(스키마)
show databases;
create database devdb; -- 새로운 데이터베이스(스키마) devdb생성
use devdb; -- devdb 데이터베이스로 들어감
select database();
show tables; -- 현재 데이터베이스 내의 테이블
create table emp(
	empno numeric(4) primary key,
    ename varchar(3) not null,  -- 글자수가 3글자(한, 영 관계 없음)
...
insert into major (mname, moffice) values ('컴공','901호');
insert into major (mname, moffice) values ('인공지능','a601호');
insert into major (mname, moffice) values ('빅데이터','b502호');
select * from major;
insert into student values (101, '홍길동', 1);
insert into student values (102, '신길동', 3);
insert into student values (103, '김길동', 4); -- 에러(FK제약조건)
select * from  student;
-- 학번, 이름, 학과번호, 학과명, 학과사무실(학생 없는 학과도 출력)
...
-- 6. 사번, 이름, 부서명
	select pno, pname, dname
        ...
        where p.dno = d.dno;
-- 7. 사번, 이름, 상사이름
	select p.pno, p.pname, mp.pname
		from personal p, personal mp
        where p.manager = mp.pno;
-- 8. 사번, 이름, 상사이름(상사가 없는 사람도 출력하되 상사가 없는 경우 ★CEO★로 출력) 
	select p.pno, p.pname, ifnull(mp.pname,'★CEO★') 'manager'
		from personal p left outer join personal mp
        on p.manager = mp.pno; 
-- 8-1 사번, 이름, 상사사번(상사가 없으면 ceo로 출력. ifnull함수의 매개변수의 타입이 상이해도 상관없음)
	select pno, pname, ifnull(manager,'CEO') 'managerNo'
		from personal;
-- 8-2. 사번, 이름, 상사이름, 부서명(상사가 없는 사람도 출력) – 같이 합시다
    select p.pno, p.pname, mp.pname "manager", d.dname
...
-- 10. 사번, 이름, 급여, 부서명, 상사이름
	select p.pno, p.pname, p.pay, dname, mp.pname
