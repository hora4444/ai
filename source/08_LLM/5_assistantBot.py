...
assistant_cs = client.beta.assistants.create(
    name='CustomerQnABot',
    instructions='당신은 고객 지원 챗봇입니다. 사용자 문의에 간략히 답변해 주세요',
    model = 'gpt-4.1-nano'
)
# 2. thread 생성 : 대화 시작
thread_cs = client.beta.threads.create()
print('어서오세요(exit를 입력하시면 종료합니다.)')
while True:
    user_input = input('User :').strip()
    if user_input.lower() in ['exit', '종료', 'end']:
...
# 6. 대화이력 data/ch7_history.txt로 파일 저장
print('이상의 대화 이력을 파일에 백업합니다.')
sorted_messages = sorted(messages.data, key = lambda msg : msg.created_at)
with open('data/ch7_history_vs.txt', 'w', encoding='utf-8') as f:
    for msg in sorted_messages:
        # role/ content /dataStr
        role = msg.role
